...
void tnpFitter::fits(bool mcTruth,string title) {
  ...
  RooFitResult* resPass = pdfPass->fitTo(*_work->data("hPass"),Minos(_useMinos),SumW2Error(kFALSE),Save(),Range("fitMassRange"));
  ...
  RooFitResult* resFail = pdfFail->fitTo(*_work->data("hFail"),Minos(_useMinos),SumW2Error(kFALSE),Save(),Range("fitMassRange"));
